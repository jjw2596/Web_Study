...
* 인터넷 상의 모든 도메인은 루트라고 불리는 도메인 아래에 계층적으로 구성

* 루트 도메인 아래의 단계를 1단계 도메인 또는 최상위 도메인(Top Level Domain)라고 부르며, 그 다음을 2단계 도메인(SLD)이라고 함

* 도메인은 일반최상위도메인(gTLD)과 국가 최상위도메인(ccTLD)로 구분할 수 있으며 gTLD는 스폰서 도메인과 언스폰서 도메인으로 구분된다.

* 스폰서 도메인은 특정한 단체를 대표하는 스폰서가 있는 최상위 도메인 (예 : xxx.app, xxx.org 등)
